...
kubectl cp configs-and-secrets/grouper/shibboleth/sp-cert.pem grouper-ws-load-data:/etc/shibboleth
kubectl cp configs-and-secrets/grouper/application/grouper.properties grouper-ws-load-data:/opt/grouper/conf
kubectl cp configs-and-secrets/grouper/application/grouper.client.properties grouper-ws-load-data:/opt/grouper/conf
kubectl cp configs-and-secrets/grouper/shibboleth/shibboleth2.xml  grouper-ws-load-data:/etc/shibboleth
kubectl cp configs-and-secrets/grouper/shibboleth/idp-metadata.xml  grouper-ws-load-data:/etc/shibboleth
kubectl cp configs-and-secrets/grouper/httpd/host-cert.pem  grouper-ws-load-data:/etc/pki/tls/certs
kubectl cp configs-and-secrets/grouper/httpd/cachain-cer.pem  grouper-ws-load-data:/etc/pki/tls/certs
kubectl cp configs-and-secrets/grouper/httpd/host-cert.pem  grouper-ws-load-data:/etc/pki/tls/certs/cachain.pem
kubectl cp configs-and-secrets/grouper/shibboleth/sp-cert.pem grouper-ui-load-data:/etc/shibboleth
...
kubectl cp configs-and-secrets/midpoint/shibboleth/sp-signing-cert.pem  midpoint-server-load-data:/etc/shibboleth-volume
kubectl cp configs-and-secrets/midpoint/shibboleth/sp-encrypt-cert.pem  midpoint-server-load-data:/etc/shibboleth-volume
