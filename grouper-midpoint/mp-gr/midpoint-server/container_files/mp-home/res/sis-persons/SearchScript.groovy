...
switch ( objectClass ) {
    case "__ACCOUNT__":
    sql.eachRow("select p.uid as uid, surname, givenName, fullName, department, mail, group_concat(affiliation) as affiliations from SIS_PERSONS p left join SIS_AFFILIATIONS a on p.uid = a.uid group by p.uid", {result.add([
	__UID__:it.uid, 
	__NAME__:it.uid, 
	uid:it.uid,
	surname:it.surname,
	...
	mail:it.mail,
	affiliation:it.affiliations?.tokenize(',')])} );
    break
